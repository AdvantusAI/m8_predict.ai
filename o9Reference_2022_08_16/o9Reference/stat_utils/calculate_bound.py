...
def calculate_bound(
    ...
    ), "Replacement Threshold should be float type ..."
    ...
    data_col = "actual"
    upper_bound_detection_col = "Upper Bound"
    lower_bound_detection_col = "Lower Bound"
    moving_avg_col = "moving avg"
    std_dev_col = "standard deviation"
    upper_bound_replacement_col = "Upper Replacement"
    lower_bound_replacement_col = "Lower Replacement"
    ...
    if method == "Fixed Sigma":
        ...
        UL_detection = mean + UpperThreshold * std_dev.iloc[0]
        LL_detection = mean - LowerThreshold * std_dev.iloc[0]

        # replacement limtis
        UL_replacement = mean + replacement_threshold * std_dev.iloc[0]
        LL_replacement = mean + replacement_threshold * std_dev.iloc[0]

        return (
            np.full(len(df), UL_detection),
            np.full(len(df), LL_detection),
            np.full(len(df), mean),
            np.full(len(df), UL_replacement),
            np.full(len(df), LL_replacement),
        )

    elif method == "Rolling Sigma":

        # calculating mean and standard deviation for each window
        df[moving_avg_col] = (
            df.iloc[:, 0].rolling(window=rolling_window_size).mean().shift(1)
        )
        df[std_dev_col] = (
            df.iloc[:, 0].rolling(window=rolling_window_size).std().shift(1)
        )

        # back filling the initial nan's
        df[moving_avg_col].fillna(method="backfill", inplace=True)
        df[std_dev_col].fillna(method="backfill", inplace=True)

        if len(df) <= rolling_window_size:
            df[moving_avg_col].fillna(df[data_col].mean(), inplace=True)
            df[std_dev_col].fillna(df[data_col].std(), inplace=True)

        print("df moving avg column: {}".format(df[moving_avg_col]))

        df[upper_bound_detection_col] = (
            df[moving_avg_col] + df[std_dev_col] * UpperThreshold
        )
        df[lower_bound_detection_col] = (
            df[moving_avg_col] - df[std_dev_col] * LowerThreshold
        )

        df[upper_bound_replacement_col] = (
            df[moving_avg_col] + df[std_dev_col] * replacement_threshold
        )
        df[lower_bound_replacement_col] = (
            df[moving_avg_col] - df[std_dev_col] * replacement_threshold
        )

        return (
            df[upper_bound_detection_col].to_numpy(),
            df[lower_bound_detection_col].to_numpy(),
            df[moving_avg_col].to_numpy(),
            df[upper_bound_replacement_col].to_numpy(),
            df[lower_bound_replacement_col].to_numpy(),
        )
        ...
        # detection bounds
        limitsthresholdIQR_detection = [
            q1 - LowerThreshold * iqr,
            q3 + UpperThreshold * iqr,
        ]

        # replacement bounds
        limitsthresholdIQR_replacement = [
            q1 - replacement_threshold * iqr,
            q3 + replacement_threshold * iqr,
        ]

        return (
            np.full(len(df), limitsthresholdIQR_detection[1]),
            np.full(len(df), limitsthresholdIQR_detection[0]),
            df[data_col].to_numpy(),
            np.full(len(df), limitsthresholdIQR_replacement[1]),
            np.full(len(df), limitsthresholdIQR_replacement[0]),
        )
