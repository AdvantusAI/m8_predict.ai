...
def get_n_time_periods(
    ...
    assert len(time_mapping) > abs(
        periods
    ), "Insufficient time mapping data to map {} periods ...".format(
        abs(periods)
    )
    ...
    assert (
        len(result) > 0
    ), "dates not found for specified periods, kindly check input time mapping ..."
...
def filter_relevant_time_mapping(
    ...
    if frequency == "Weekly":
        time_mapping = time_mapping[["WeekName", "WeekKey"]]
    elif frequency == "Monthly":
        time_mapping = time_mapping[["MonthName", "MonthKey"]]
    elif frequency == "Quarterly":
        time_mapping = time_mapping[["QuarterName", "QuarterKey"]]
    else:
        raise ValueError(
