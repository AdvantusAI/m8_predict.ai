...
def fill_missing_dates(
    ...
    for the_col in forecast_level:
        assert the_col in actual.columns, "{} not present in actual".format(
            the_col
        )

    assert time_level in actual.columns, "time_level not present in actual ..."
    ...
    relevant_actuals_nas_filled[
        relevant_time_key
    ] = relevant_actuals_nas_filled[time_level].map(date_name_to_key_mapping)
