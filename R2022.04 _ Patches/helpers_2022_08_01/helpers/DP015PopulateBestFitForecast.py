...
import pandas as pd
from o9Reference.common_utils.dataframe_utils import get_o9_empty_df
...
def main(
    ...
    try:
        ...
        if ForecastBounds.empty:
            logger.warning(
                "ForecastBounds dataframe is empty for slice {}, will populate NaN into {}/{} columns".format(
                    df_keys, best_fit_ub_col, best_fit_lb_col
                )
            )
            # if bounds are empty, populate NaNs into the 80 % LB and UB Columns
            AllForecast[best_fit_ub_col] = np.nan
            AllForecast[best_fit_lb_col] = np.nan
        else:
        ...
        logger.exception(e)
        BestFitForecast = get_o9_empty_df()
