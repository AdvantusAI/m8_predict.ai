...
def cleanse_data_wrapper(
    ...
    try:
        ...
        if len(df) > 1:
            ...
            # in case series has only one data point, we populate same
            existing_value = df[history_measure].iloc[0]
            cleansed_data = np.array(existing_value)
            upper_bound = np.array(existing_value)
            lower_bound = np.array(existing_value)
            mean = np.array(existing_value)
