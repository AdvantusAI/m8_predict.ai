...
def main(
    ...
    try:
        ...
        AllForecast, ForecastModel = (
            pd.DataFrame(columns=dummy_output_cols),
            pd.DataFrame(columns=dummy_output_cols),
        )
        ...
        if confidence_interval_alpha > 1:
            logger.warning(
                "confidence interval parameter should be less than one ..."
            )
            logger.warning("overriding with default value of 0.2")
        ...
        logger.exception(e)
        AllForecast = pd.DataFrame(columns=dummy_output_cols)
