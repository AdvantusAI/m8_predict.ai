...
# TODO: Include updated seasonality detection from Numbers POD
...
def main(
    ...
    # Define dummy output columns and df to avoid parsing errors on PythonServer
    ...
    try:
        ...
        logger.info("--------- DimClass : {}".format(DimClass))
        logger.info("--------- VolSegLvl : {}".format(VolSegLvl))
        logger.info("--------- COVSegLvl : {}".format(COVSegLvl))
        logger.info(
            "--------- VolumeThresholdMeasure : {}".format(
                VolumeThresholdMeasure
            )
        )
        logger.info(
            "--------- COVThresholdMeasure : {}".format(COVThresholdMeasure)
        )
        ...
        ProductSegmentation = pd.DataFrame(
            columns=prod_segmentation_cols_output
        )
        ...
        cov_thresholds = [
            float(x) for x in list(SegmentThresholds[COVThresholdMeasure])
        ]
        ...
        vol_thresholds = [
            float(x) for x in list(SegmentThresholds[VolumeThresholdMeasure])
        ]
        ...
        if len(vol_cov_segmentation_input) > 0:
            ...
            def get_mean_stddev(values: List[float], std_dev_col, mean_col):
        ...
        logger.exception(e)
        StatSegmentation = pd.DataFrame(columns=dummy_output_cols)
        ProductSegmentation = pd.DataFrame(columns=dummy_output_cols)
