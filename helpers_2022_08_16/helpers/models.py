            ...
            try:
                if the_model_name == "Seasonal Naive YoY":
                    ...
                    if len(train) < seasonal_periods:
                        ...
                        num_points_to_pad = seasonal_periods - len(train)
                    ...
                    if in_sample_flag:
                        ...
                        last_n_values = snaive_train.tail(
                            seasonal_periods
                        ).values
                ...
                elif the_model_name in ["Linear Regression", "KNN Regression"]:
                    # AR - NNET is not available in Python, implementing a Linear Regression model which uses past 6 periods
                    if in_sample_flag:
                        # in sample predictions are not yet implemented for sktime make_reduction
                        pass
                        ...
                        total_length = len(train) + forecast_horizon
                        ...
                        x_train_test = pd.DataFrame(
                            {
                                trend_idx_col: [
                                    x for x in range(1, total_length + 1)
                                ]
                            }
                        )

                        # create repeatable seasonal index
                        x_train_test[seasonal_idx_col] = x_train_test[
                            trend_idx_col
                        ].mod(seasonal_periods)

                        # take sin cos components
                        x_train_test[sin_seasonal_idx_col] = np.sin(
                            x_train_test[seasonal_idx_col]
                            / seasonal_periods
                            * 2
                            * np.pi
                        )
                        ...
                        x_train_test.drop(
                            seasonal_idx_col, axis=1, inplace=True
                        )

                        # normalize trend
                        x_train_test[trend_idx_col] = (
                            x_train_test[trend_idx_col]
                            / x_train_test[trend_idx_col].max()
                        )

                        # separate into train and test sets
                        x_train = x_train_test.head(len(train))
                        x_test = x_train_test.tail(forecast_horizon)

                        # fit model
                        the_estimator.fit(train, X=x_train)

                        # get predictions, confidence interval is not available
                        the_forecast = the_estimator.predict(
                            X=x_test, fh=fcst_horizon
                        )
                    ...
                    stlf_train_test_index = pd.date_range(
                        start=random_start_date,
                        periods=len(train) + forecast_horizon,
                        freq=get_ts_freq_prophet(seasonal_periods),
                    )
                    ...
                    the_forecast = pd.Series(
                        the_stlf_forecast_df["mean"].values
                    )
                ...
                elif the_model_name in ["SES", "DES", "TES"]:
                    ...
                    if in_sample_flag:
                        ...
                        ).summary_frame(alpha=confidence_interval_alpha)
                    else:
                        the_pred_with_bounds = the_estimator.get_prediction(
                            start=end_index + 1,
                            end=end_index + forecast_horizon,
                        ).summary_frame(alpha=confidence_interval_alpha)

                    the_forecast = pd.Series(
                        the_pred_with_bounds["mean"].values
                    )
                    ...
                    if the_model_name == "TBATS" and in_sample_flag:
...
def train_models_for_one_intersection(
    ...
    try:
        ...
        logger.info("---- validation_method : {}".format(validation_method))

        logger.info("---- PASS 1 : Fitting models ...")
        logger.info("---- train dataset size : {}".format(len(train)))

        # convert both time series to float 64 for all models to work
        ...
        # extract keys from pass_1_estimators
        valid_algos_pass_1 = [x[0] for x in pass_1_estimators]
        logger.info("---- valid algorithms : {}".format(valid_algos_pass_1))
        ...
        if not valid_model_desc_df.empty:
            # Add dimension columns
            for the_index, the_level in enumerate(forecast_level):
        ...
        logger.info("---- train dataset size : {}".format(len(time_series)))
        ...
        # extract keys from pass_1_estimators
        valid_algos_pass_2 = [x[0] for x in pass_2_estimators]
        logger.info("---- valid algorithms : {}".format(valid_algos_pass_2))
...
def get_valid_estimators(
    ...
    seasonal_estimators = [
        ...
        (
            "KNN Regression",
            KNeighborsRegressor(n_neighbors=seasonal_periods // 3),
        ),
    ]
    ...
    if train_size > seasonal_periods:
        estimators = (
            non_seasonal_estimators + seasonal_estimators + tbats_estimator
        )
